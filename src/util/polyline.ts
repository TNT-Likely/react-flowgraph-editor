 ...
 */
const substitute = (str: string, o: any) => {
  if (!str || !o) {
    return str;
  }
...
export const isBBoxesOverlapping = (b1: any, b2: any) =>
...
export const isHorizontalPort = (port: PolyPoint, bbox: any): boolean => {
 ...
 * @param b2
 */
export const mergeBBox = (b1: any, b2: any): PBBox => {
...
export const getBBoxXCrossPoints = (bbox: PBBox, x: number): PolyPoint[] => {
  if (x < (bbox.minX || 0) || x > (bbox.maxX || 0)) {
    return [];
  }
  return [
    {
...
export const getBBoxYCrossPoints = (bbox: PBBox, y: number): PolyPoint[] => {
  if (y < (bbox.minY || 0) || y > (bbox.maxY || 0)) {
    return [];
  }
  return [
    {
...
export const getPolylinePoints = (
  ...
  if (!sNode || !sNode.getType()) {
    ...
    sBBox.centerX = (sBBox.minX || 0 + (sBBox.maxX || 0)) / 2;
    sBBox.centerY = (sBBox.minY || 0 + (sBBox.maxY || 0)) / 2;
  } else {
    sBBox = sNode.getBBox();
  }
  ...
  if (!tNode || !tNode.getType()) {
    ...
    tBBox.centerX = (tBBox.minX || 0 + (tBBox.maxX || 0)) / 2;
    tBBox.centerY = (tBBox.minY || 0 + (tBBox.maxY || 0)) / 2;
  } else {
  ...
  [lineBBox, sMixBBox, tMixBBox].forEach((bbox: PBBox) => {
    connectPoints = connectPoints.concat(
      getBBoxCrossPointsByPoint(bbox as any, centerPoint).filter(
        p =>
          isPointOutsideBBox(p, sxBBox as any) &&
          isPointOutsideBBox(p, txBBox as any),
      ),
    );
  });
  [
